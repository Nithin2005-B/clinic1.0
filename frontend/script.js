...
if (contactForm) {
  contactForm.addEventListener("submit", function (e) {
    ...
    if (!name || !email || !phone) return alert("⚠️ Please fill out all required fields.");
    if (!/^[^ ]+@[^ ]+\.[a-z]{2,3}$/.test(email)) return alert("⚠️ Invalid email.");
    if (!/^[0-9]{10}$/.test(phone)) return alert("⚠️ Invalid phone.");
...
async function loadBookings() {
  ...
  try {
    ...
    if (!data.success || !data.bookings || data.bookings.length === 0) {
      bookingsTable.innerHTML = `<tr><td colspan="7">⚠️ No bookings found</td></tr>`;
      return;
    }
    ...
    data.bookings.forEach(b => {
      ...
      // Optional: click row to auto-fill reply form
    ...
    console.error(err);
    bookingsTable.innerHTML = `<tr><td colspan="7">⚠️ Error loading bookings</td></tr>`;
  }
}
